...
export type Action =
  ...
  | { type: ActionKind.ITEM_MOVED, id: string, oldPosition: number, newPosition: number }
...
export const reducer = (state: State, action: Action) => {
  switch (action.type) {
    ...
    case ActionKind.NEW_LIST:
      return { ...state, list: { id: action.id, items: [], name: '' }, saved: false }
    case ActionKind.LIST_NAME_CHANGED:
      return { ...state, list: { ...state.list, name: action.name }, saved: false }
    case ActionKind.ITEM_TEXT_CHANGED:
      return {
        ...
        {
          ...state.list,
          items: state.list.items.map(i => i.id === action.id ? { ...i, description: action.text } : i)
        },
        saved: false
      }
    case ActionKind.ITEM_STATE_CHANGED:
      return {
        ...
        {
          ...state.list,
          items: state.list.items.map(i => i.id === action.id ? { ...i, done: action.checked } : i)
        },
        saved: false
      };
    case ActionKind.ITEM_REMOVED:
      return { ...state, list: { ...state.list, items: state.list.items.filter(i => i.id !== action.id) }, saved: false };
    case ActionKind.ITEM_ADDED:
      return {
        ...state,
        list: {
          ...state.list,
          items: [...state.list.items, { id: crypto.randomUUID(), description: '', done: false, order: -1 }].map((item, index) => ({ ...item, order: index })),
        },
        saved: false
      };
    case ActionKind.ITEM_MOVED:
      if (action.newPosition < 0 || action.newPosition >= state.list.items.length)
        return state;
      return {
        ...state,
        list: {
