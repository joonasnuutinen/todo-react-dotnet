...
const App = () => {
  ...
  return (
    <TodoList
      ...
      itemStateChanged={(id, value) =>
        dispatch({ type: ActionKind.ITEM_STATE_CHANGED, id, checked: value })
      }
      itemTextChanged={(id, text) =>
        dispatch({ type: ActionKind.ITEM_TEXT_CHANGED, id, text })
      }
      itemRemoved={(id) => dispatch({ type: ActionKind.ITEM_REMOVED, id })}
      itemAdded={() => dispatch({ type: ActionKind.ITEM_ADDED })}
      listNameChanged={(text) =>
        dispatch({ type: ActionKind.LIST_NAME_CHANGED, name: text })
      }
      itemMoved={(id, oldPosition, newPosition) =>
        dispatch({ type: ActionKind.ITEM_MOVED, id, oldPosition, newPosition })
      }
    />
  );
};
